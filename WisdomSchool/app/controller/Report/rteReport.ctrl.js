(function () {
    'use strict';
    var controllerId = 'rteReportCtrl';
    ...
    function rteReportCtrlFn($scope, ReportSrvc, Excel, $timeout, StudentDetailservice, $location) {
        $scope.heading = "RTE Report";
        ...
        function rteDetail(data)
        {
            var rteList = [];
            var id = 0;
            angular.forEach(data, function (item) {
                if (item.RTE === "1" || item.RTE === "True")
                {
                    rteList.push(item);
                    id = id + 1;
                }
            });
            $scope.rteStudents = rteList;
        }
